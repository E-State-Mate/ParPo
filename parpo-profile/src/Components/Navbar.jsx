  ...
  return (
    <Grid container id="navbar" direction="row" alignItems="center" style={{ backgroundColor: "#5CA8B2" }}>
      ...
      <Grid item sm={4} sx={{ display: { md: "none" } }}>
        ...
        >
          {links.map((link, i) => (
            <MenuItem key={i} component={Link} to={link.route}>
              {link.label}
            </MenuItem>
          ))}
        </Menu>
      ...
      {links.map((link, i) => (
        <Grid key={i} item md sx={{ display: { xs: "none", md: "flex" } }} justifyContent="center">
          {/* {link.route === '/warehouse' ? 
                    ...
                    :  */}
          <Button component={Link} to={link.route} style={{ textDecoration: "none", color: "white" }}>
            {link.label}
          </Button>
          {/* } */}
        </Grid>
